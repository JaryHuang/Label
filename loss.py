...
class CrossEntropyLoss(Module):
    ...
    def getLabelMatrix(self,label):
        ...
        one_hot = torch.Tensor([[[0 if i!=j else 1 for i in range(self.ClassNum) ] for j in label[k]] for k in range(batch)]).reshape(-1,self.ClassNum).cuda()
...
class SelNLPL(CrossEntropyLoss):
    ...
    def __init__(self, class_num, c, r, negative_num=1, reduction="mean",**kwage):
        ...
        self.labelG = LabelGeneration(class_num =class_num, negative_num = negative_num) 
    ...
    def forward(self, pred, label, mode):
        ...
        if torch.sum(mask) == 0:
            ...
            nl_one_hot, batch, nlnum = self.getLabelMatrix(torch.Tensor(nl_label))
            # SelNL calculation
            prb = prob[mask].unsqueeze(1).repeat(1,nlnum,1).reshape(-1,self.ClassNum) 
...
class LabelSmooth(CrossEntropyLoss):
    ...
    def getLabelMatrix(self,label):
        N,M = label.size()
        one_hot = torch.Tensor([[[0 if i!=j else 1 for i in range(self.ClassNum) ] for j in label[k]] for k in range(N)]).reshape(-1,self.ClassNum).cuda()
        one_hot = one_hot * (1-self.alpha) +(1-one_hot) * self.alpha/(self.ClassNum-1)
        return one_hot, N, M
